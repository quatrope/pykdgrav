...
class KDNode(object):
    def __init__(self, points, masses, softening):
        ...
        if self.Npoints == 1:
            ...
            for k in range(3):
                ...
                self.delta += (
                    0.5 * (self.bounds[k, 1] + self.bounds[k, 0]) - self.COM[k]
                ) ** 2
    ...
    def GenerateChildren(self, axis):
        ...
        if np.any(index):
            self.left = KDNode(
                self.points[index], self.masses[index], self.softening[index]
            )
            self.HasLeft = True
...
def ConstructKDTree(x, m, softening):
    ...
    while divisible_nodes > 0:
        ...
        for i in range(
            count, N
        ):  # loop through the nodes we spawned in the previous pass
            count += 1
            if nodes[i].IsLeaf:
                continue
            else:
