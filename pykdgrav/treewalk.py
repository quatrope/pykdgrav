...
% accuracy)
    ...
    if r > 0:
        if node.IsLeaf:
            ...
            node.size / theta + node.delta, max(node.h, softening) + node.size
        ):
            add_accel = True
            fac = node.mass / (r * r * r)
...
def GetPotentialParallel(pos, tree, G, theta):
    ...
    for i in prange(pos.shape[0]):
        result[i] = G * PotentialWalk(pos[i], tree, 0.0, theta=theta)
    return result
...
@njit(fastmath=True)
def GetAccel(pos, tree, softening=None, G=1.0, theta=0.7):
    if softening is None:
        softening = zeros(pos.shape[0])
    result = empty(pos.shape)
    for i in range(pos.shape[0]):
        result[i] = G * ForceWalk(
            pos[i], tree, zeros(3), softening=softening[i], theta=theta
        )
    return result


@njit(parallel=True, fastmath=True)
def GetAccelParallel(pos, tree, softening, G=1.0, theta=0.7):
    if softening is None:
        softening = zeros(len(pos), dtype=np.float64)
    result = empty(pos.shape)
    for i in prange(pos.shape[0]):
        result[i] = G * ForceWalk(
            pos[i], tree, zeros(3), softening=softening[i], theta=theta
